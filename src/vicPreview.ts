...
export class VicPreview extends Disposable {
  ...
  private getWebviewContents(): string {
    ...
    const config = vscode.workspace.getConfiguration('vic-preview');
    const settings = {
...
<link rel="stylesheet" href="${escapeAttribute(
      this.extensionResource('/vic/main.css')
...
<body class="container image scale-to-fit loading">
  ...
  <script src="${escapeAttribute(
    this.extensionResource('/vic/vic_loader.js')
  )}" nonce="${nonce}"></script>
</body>`;
